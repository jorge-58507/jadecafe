...
class general_funct {
  ...
  isEmpty(field, set_class = true) {
    if (field.value.length === 0 || /^\s+$/.test(field.value)) {
      if (set_class) {
        this.set_invalid([field]);
      }
      return 0; //Vacio
    } else {
      if (set_class) {
        this.set_valid([field]);
      }
      return 1; //Lleno
    }
  }
  ...
  /*async async_laravel_request(url, method, funcion, body_json = '') 
  {
  ...
  async async_laravel_request(url, method, funcion, body_json = "") {
    ...
    // 🔁 Detectar redirección real (login, middleware, etc.)
    if (response.redirected) {
      window.location.href = response.url;
      return false;
    }

    // ❌ Error HTTP (pero sin romper llamados existentes)
    if (!response.ok) {
      console.error("HTTP error:", response.status);
      return false;
    }

    // ✅ Siempre JSON (según tu regla)
    let json_obj = await response.json();

    if (json_obj && typeof funcion === "function") {
      return funcion(json_obj);
    }
  }
  ...
  validFranz(selector, raw_acept, alt = "") {
    // raw_acept = array; selector = "string"
    var characters = "";
  ...
  open_datepicker(originDate) {
    ...
    var instances_datepicker = M.Datepicker.init(datepicker, {
      autoClose: true,
      format: "dd-mm-yyyy",
      container: "body",
  ...
  check_invalid(inputs) {
    // inputs es un array que contiene los name de los campos
  ...
  val_price(str, decimal = 2, refill = 1, split = 1) {
    ...
    if (str_splited.length > 1) {
      if (str_splited.length > 2) {
        //Si esta mal escrito y tiene 2 puntos es decir 2 grupos decimales -> reducir a 1 grupo decimal
        str_splited.splice(2);
      }
      if (str_splited[0].length === 0) {
        //Si en cambio no tiene ni decimal ni entero igualar a cero
        str_splited[0] = "0";
      }
      if (refill === 1) {
  ...
  val_dec(str, decimal, refill, split) {
    ...
    if (str_splited.length > 1) {
      //SI STRING TIENE PARTE DECIMAL
      if (str_splited.length > 2) {
        //SI TIENE DOBLE PUNTO CORREGIR
        str_splited.splice(2);
      }
      if (str_splited[0].length === 0) {
        //SI NO TIENE NUMERO ENTERO, AGREGAR CERO
        str_splited[0] = "0";
      }
      if (refill === 1) {
        //SI RELLENAR ES 1, AGREGARLE CEROS ALFINAL
        str_splited[1] += decimal_part; // REFILL
      }

      if (split === 1) {
        //SI RECORTAR EL STRING ES 1, REALIZAR EL CORTE
