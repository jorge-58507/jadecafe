...
class class_request {
  ...
  render(target, raw) {
    switch (target) {
      case "open":
        document.getElementById("container_openrequest").innerHTML =
          cls_request.generate_openrequest(raw);
        break;
      case "closed":
        document.getElementById("container_closedrequest").innerHTML =
          cls_request.generate_closedrequest(raw);
        break;
      case "canceled":
        document.getElementById("container_canceledrequest").innerHTML =
          cls_request.generate_canceledrequest(raw);
        break;
      case "online":
        var list = cls_request.generate_onlinerequest(raw);
        if (document.getElementById("container_onlinerequest")) {
          document.getElementById("container_onlinerequest").innerHTML =
            list.content;
  ...
  generate_onlinerequest(raw_request) {
    ...
    raw_request.map((request) => {
      ...
      var table =
        cls_general.is_empty_var(request.tx_table_value) === 0
          ? "Sin Mesa"
          : request.tx_table_value;
  ...
  close_inaction(btn) {
    ...
    if (cls_general.is_empty_var(request_slug) === 0) {
      cls_general.shot_toast_bs("No hay elementos para cerrar.", {
        bg: "text-bg-warning",
      });
      return false;
    }
  ...
  async filter(target, str) {
    switch (target) {
      case "closed":
        document.getElementById("container_closedrequest").innerHTML =
          cls_request.generate_closedrequest(
            await cls_request.look_for(str, "1"),
          );
        break;
      case "open":
        document.getElementById("container_openrequest").innerHTML =
          cls_request.generate_openrequest(
            await cls_request.look_for(str, "0"),
          );
        break;
      case "canceled":
        document.getElementById("container_canceledrequest").innerHTML =
          cls_request.generate_canceledrequest(await cls_charge.look_for(str));
        break;
      case "online":
        var list = cls_request.generate_onlinerequest(
          await cls_request.look_for(str, "2"),
        );
        document.getElementById("container_onlinerequest").innerHTML =
          list.content;
        break;
      ...
      this.timer = setTimeout(function () {
        ...
        for (var i in haystack) {
          ...
          for (const a in needles) {
            haystack[i]["tx_table_value"] =
              cls_general.is_empty_var(haystack[i]["tx_table_value"]) === 0
                ? ""
                : haystack[i]["tx_table_value"];
            if (
              haystack[i]["tx_client_name"]
                .toLowerCase()
                .indexOf(needles[a].toLowerCase()) > -1 ||
              haystack[i]["tx_request_code"]
                .toLowerCase()
                .indexOf(needles[a].toLowerCase()) > -1 ||
              haystack[i]["tx_request_title"]
                .toLowerCase()
                .indexOf(needles[a].toLowerCase()) > -1 ||
              haystack[i]["tx_table_value"]
                .toLowerCase()
  ...
  create() {
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        ...
        if (bar_slug === "") {
          cls_general.shot_toast_bs("No existe alguna barra activa.", {
            bg: "text-bg-warning",
          });
        }
  ...
  inspect_information(raw_data) {
    ...
    command_procesed.map((command) => {
      ...
      var observation =
        cls_general.is_empty_var(command.tx_command_observation) === 1
          ? ", <strong>Obs.</strong> " + command.tx_command_observation
          : "";
      if (command.tx_commanddata_status === 0) {
        ...
        raw_price.push({
          price: command.tx_commanddata_price,
          discount: command.tx_commanddata_discountrate,
          tax: command.tx_commanddata_taxrate,
          quantity: command.tx_commanddata_quantity,
        });
        var bg_status = "";
      }
    ...
    command_related.map((command) => {
      var raw_option =
        cls_general.is_empty_var(command.tx_localcommanddata_option) === 0
    ...
    document.getElementById("onlinerequestModal_title").innerHTML =
      "Inspeccionar Pedido";
...
class class_charge {
  ...
  index() {
    ...
    document
      .getElementById("btn_onlinerequest")
    ...
    document
      .getElementById("btn_filteronlinerequest")
      .addEventListener("click", () => {
    ...
    var funcion = async function (obj) {
      if (obj.status === "success") {
        ...
        document.getElementById("sp_discount").innerHTML =
          "B/. " + cls_general.val_price(price.discount, 2, 1, 1);
        document.getElementById("sp_subtotal").innerHTML =
          "B/. " + cls_general.val_price(price.subtotal, 2, 1, 1);
        document.getElementById("sp_tax").innerHTML =
          "B/. " + cls_general.val_price(price.tax, 2, 1, 1);
        document.getElementById("sp_total").innerHTML =
          "B/. " + cls_general.val_price(price.total, 2, 1, 1);

        cls_charge.charge_request = {
          request_id: obj.data.request_info.ai_request_id,
          gross_total: price.gross_total,
  ...
  render(request_slug) {
    ...
    document.getElementById("container_request").innerHTML = content;
    document
      .getElementById("btn_paymentProcess")
      .addEventListener("click", () => {
        cls_payment.process(
          document.getElementById("btn_paymentProcess"),
          request_slug,
        );
      });
    cls_paymentmethod.render();

    document.getElementById("btn_graphicList").addEventListener("click", () => {
      cls_command.filter_articlethumbnail("");
    });

    var raw_category = [];
    cls_article.article_list.map((article) => {
      var cat = raw_category.find((category) => {
        return category === article.tx_category_value;
      });
    ...
    document.getElementById("container_articlethumbnail_categories").innerHTML =
      content_categorythumbnail;
  ...
  look_for(str) {
    return new Promise((resolve) => {
      ...
      this.timer = setTimeout(function () {
        ...
        let fechaFrom = new Date(
          cls_general.date_converter("dmy", "ymd", date_i),
        );
        let fechaTo = new Date(
          cls_general.date_converter("dmy", "ymd", date_f),
        );
        let yearDiff = fechaTo.getFullYear() - fechaFrom.getFullYear();
        let MonthDiff = fechaTo.getMonth() - fechaFrom.getMonth();
        if (yearDiff > 0) {
          cls_general.shot_toast_bs(
            "El rango de fecha debe ser maximo 60 dias.",
            { bg: "text-bg-warning" },
          );
          return false;
        }
        ...
        if (
          cls_general.is_empty_var(date_i) === 0 ||
          cls_general.is_empty_var(date_f) === 0
        ) {
          cls_general.shot_toast_bs("Ingrese las fechas.", {
            bg: "text-bg-warning",
          });
          return false;
        }
  ...
  inspect(charge_slug) {
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        ...
        cls_charge.render_inspect(
          charge.tx_charge_number,
          cls_general.datetime_converter(charge.created_at),
          charge.tx_client_name,
          charge.tx_charge_total,
          charge.tx_charge_change,
          raw_payment,
          raw_article,
          charge_slug,
          charge.tx_charge_tip,
        );
  ...
  render_inspect(
    ...
    raw_payment.map((payment) => {
      var payment_number =
        cls_general.is_empty_var(payment.tx_payment_number) === 0
          ? ""
          : "- (" + payment.tx_payment_number + ")";
      payment_list += `
    ...
    raw_article.map((article) => {
      var discount =
        (article.tx_commanddata_discountrate * article.tx_commanddata_price) /
        100;
      discount = discount.toFixed(2);
      var price = article.tx_commanddata_price - parseFloat(discount);
      var bg_status =
        article.tx_commanddata_status === 0
          ? "text-bg-danger"
          : "text-bg-success";
      article_list += `
    ...
    document.getElementById("inspectModal_title").innerHTML =
      "Inspeccionar Venta " + number;

    document
      .getElementById("btn_chargeCreditnote")
      .addEventListener("click", () => {
        cls_charge.loginuser_creditnote(charge_slug);
      });
    document.getElementById("btn_chargePrint").addEventListener("click", () => {
      cls_charge.loginuser_reprint(charge_slug);
    });
  } 
  ...
  checklogin_creditnote() {
    ...
    if (
      cls_general.is_empty_var(email) === 0 ||
      cls_general.is_empty_var(password) === 0
    ) {
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        ...
        const modal_inspect = bootstrap.Modal.getInstance(
          "#login_creditnoteModal",
        );
        modal_inspect.hide();
      } else {
        cls_general.shot_toast_bs(obj.message, { bg: "text-bg-warning" });
        ...
        obj.data.article.map((command) => {
          command.map((commanddata) => {
            raw_article.push({
              ...
              commanddata_presentation_id:
                commanddata.commanddata_ai_presentation_id,
              ...
              commanddata_cnquantity:
                cls_general.is_empty_var(commanddata.sum) === 1
                  ? commanddata.sum
                  : 0,
  ...
  set_tip(suggested_tip, tip) {
    if (tip === 0) {
      if (isNaN(suggested_tip) || suggested_tip > 99) {
        cls_general.shot_toast_bs("Debe ingresar una tasa valida.");
        return false;
      }
    ...
    tip = parseFloat(tip);
    var new_total =
      parseFloat(cls_charge.charge_request.subtotal) +
      tip +
      parseFloat(cls_charge.charge_request.tax);
    ((new_total = cls_general.val_price(new_total)),
      (cls_charge.charge_request.total = parseFloat(new_total)));
    cls_charge.tip = tip;
    document.getElementById("sp_tip").innerHTML =
      "B/. " + cls_general.val_price(tip);
    document.getElementById("sp_total").innerHTML =
      "B/. " + cls_general.val_price(cls_charge.charge_request.total);
...
class class_command {
  ...
  index(request_info, table_slug) {
    ...
    cls_table.table_list.map((table) => {
      if (table.tx_table_active === 1) {
        opt_tablelist +=
          table.tx_table_slug === table_slug
            ? `<option value="${table.ai_table_id}" selected>${table.tx_table_code} - ${table.tx_table_value}</option>`
            : `<option value="${table.ai_table_id}">${table.tx_table_code} - ${table.tx_table_value}</option>`;
      }
    });
    ...
    var content_command_procesed = cls_command.generate_articleprocesed_newsale(
      cls_command.command_procesed,
    );
        ...
        if (btn) {
          ...
          var funcion = async function (obj) {
            ...
            var content_command_procesed =
              cls_command.generate_articleprocesed_newsale(
                cls_command.command_procesed,
              );
            var total_sale = await cls_general.calculate_sale(
              content_command_procesed.price,
            );
            document.getElementById("commandList").innerHTML =
              content_command_procesed.content;
            document.getElementById("requestTotal").innerHTML =
              `B/ ${cls_general.val_price(total_sale.total, 2, 1, 1)}`;
          };
          cls_general.async_laravel_request(url, method, funcion, body);
        }
      }, 15000);
  ...
  generate_articlethumbnail_list(filtered) {
    ...
    filtered.map((article) => {
      ...
      var img =
        cls_general.is_empty_var(article["tx_article_thumbnail"]) === 1
          ? `<img src="attached/image/article/${article["tx_article_thumbnail"]}" width="80px"></img>`
          : `
  ...
  show_article(article_slug, description, caller) {
    ...
    var funcion = function (obj) {
      ...
      var content_recipe = cls_command.generate_recipe_option(
        obj.data.articleproduct,
      );
      ...
      document.getElementById("commandModal_title").innerHTML =
      ...
      cls_command.modal_set_price(
        obj.data.price[0].tx_price_three +
          "," +
          obj.data.price[0].tx_price_two +
          "," +
          obj.data.price[0].tx_price_one,
        obj.data.price[0].ai_presentation_id,
        article_slug,
      ); //OPTION PARA Los PRECIOS DEL ARTICULO
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        ...
        document.getElementById("container_price").innerHTML =
          content + "</select>";
        document.getElementById("container_recipe").innerHTML =
          cls_command.generate_recipe_option(obj.data.recipe);
      } else {
        cls_general.shot_toast_bs(obj.message, { bg: "text-bg-warning" });
      }
    };
  ...
  add_article_lastrequest(article_slug, description, article_id) {
    ...
    if (isNaN(quantity) || quantity < 1) {
      cls_general.shot_toast_bs("Corrija la cantidad.", {
        bg: "text-bg-warning",
      });
      return false;
    }
    ...
    var request_slug = document.getElementById("btn_graphicList")
      ? document.getElementById("btn_graphicList").name
      : "";
    ...
    if (cls_general.is_empty_var(request_slug) === 0) {
      cls_command.command_list.push({
        ...
        presentation_value:
          select_presentation.options[select_presentation.selectedIndex].text,
  ...
  async generate_articleselected(command_list) {
    ...
    command_list.map((article, index) => {
      ...
      raw_price.push({
        price: article.price,
        discount: article.discount_rate,
        tax: article.tax_rate,
  ...
  save(request_slug, table_slug) {
    //ESTA FUNCION SOLO ABRE EL MODAL
    var command_list = cls_command.command_list;
    if (command_list < 1) {
      cls_general.shot_toast_bs("Seleccione los art&iacute;culos.", {
        bg: "text-bg-warning",
      });
      return false;
    }
  ...
  store(table_slug, title) {
    ...
    var funcion = async function (obj) {
      if (obj.status === "success") {
        ...
        document.getElementById("container_buttonUpdateInfo").innerHTML =
          `<button class="btn btn-lg btn-info" type="button" onclick="cls_general.disable_submit(this); cls_request.update_info('${obj.data.request.tx_request_slug}')">Actualizar</button>`;
        cls_command.command_procesed = obj.data.command_procesed;
        ...
        if (obj.data.cashier === 1) {
          swal({
            ...
            switch (ans) {
              case "si":
                var request_slug =
                  document.getElementById("btn_commandprocess").name;
                var url = "/request/" + request_slug + "/close";
                var method = "PUT";
                var body = JSON.stringify({ a: 1 });
                var funcion = function (obj) {
  ...
  save_online(request_slug, table_slug, onlinerequest_slug) {
    //ESTA FUNCION SOLO ABRE EL MODAL
    var command_list = cls_command.command_list;
    if (command_list < 1) {
      cls_general.shot_toast_bs("Seleccione los art&iacute;culos.", {
  ...
  store_online(table_slug, title, onlinerequest_slug) {
    cls_general.disable_submit(
      document.getElementById("btn_commandprocess"),
      0,
    );
    var command_list = cls_command.command_list;
    var client = document.getElementById("requestClient");
    ...
    var body = JSON.stringify({
      ...
      e: consumption,
      f: observation,
      g: onlinerequest_slug,
      h: cls_charge.api_token,
    });
    var funcion = function (obj) {
      if (obj.status === "success") {
        document.getElementById("btn_commandprocess").name =
          obj.data.request.tx_request_slug;
        document.getElementById("container_buttonUpdateInfo").innerHTML =
          `<button class="btn btn-lg btn-info" type="button" onclick="cls_general.disable_submit(this); cls_request.update_info('${obj.data.request.tx_request_slug}')">Actualizar</button>`;
        cls_command.command_procesed = obj.data.command_procesed;
        cls_command.command_list = [];
        ...
        cls_command.render_articleselected();
        var total_sale = cls_general.calculate_sale(
          content_command_procesed.price,
        );
        ...
        if (obj.message === "La mesa esta ocupada.") {
          cls_command.update_online(
            obj.data.tx_request_slug,
            onlinerequest_slug,
          );
        } else {
          document.getElementById("btn_commandprocess").disabled = false;
...
class class_article {
  ...
  look_for(str, limit) {
    ...
    for (var i in haystack) {
      ...
      for (const a in needles) {
        if (
          haystack[i]["tx_article_code"]
            .toLowerCase()
            .indexOf(needles[a].toLowerCase()) > -1 ||
          haystack[i]["tx_article_value"]
            .toLowerCase()
            .indexOf(needles[a].toLowerCase()) > -1
        ) {
          ocurrencys++;
...
class class_paymentmethod {
  ...
  render() {
    document.getElementById("container_paymentMethod").innerHTML =
      cls_paymentmethod.generate_paymentbutton(
        cls_paymentmethod.paymentmethod,
      ) +
      `
...
class class_payment {
  ...
  add_giftcard(giftcard, amount) {
    if (cls_general.is_empty_var(amount) === 0) {
      //VERIFICA QUE EL MONTO INGRESADO NO ESTE VACIO
      cls_general.shot_toast_bs("Ingrese el monto.", { bg: "text-bg-warning" });
      return false;
    }
    if (isNaN(amount)) {
      //VERIFICA EL MONTO INGRESADO ES UN NUMERO
      cls_general.shot_toast_bs("Monto erroneo.", { bg: "text-bg-warning" });
      return false;
    }
    var received = 0;
    cls_payment.payment.map((pay) => {
      received += parseFloat(pay.amount);
    });
    cls_payment.giftcard.map((pay) => {
      received += parseFloat(pay.amount);
    });
    ...
    if (total <= received) {
      ...
      if (received + amount > total) {
        cls_general.shot_toast_bs("M&eacute;todo de pago, no admite cambio.", {
          bg: "text-bg-warning",
        });
        return false;
      }
    }
    cls_payment.giftcard.push({
      giftcard_id: giftcard.ai_giftcard_id,
      giftcard_number: giftcard.tx_giftcard_number,
      method_name: "Cupon",
      amount: amount,
    });
  ...
  add(method) {
    ...
    var check_method = cls_paymentmethod.paymentmethod.find((pmethod) => {
      return pmethod.ai_paymentmethod_id === method;
    });
    if (cls_general.is_empty_var(amount) === 0) {
      //VERIFICA QUE EL MONTO INGRESADO NO ESTE VACIO
      cls_general.shot_toast_bs("Ingrese el monto.", { bg: "text-bg-warning" });
    ...
    if (cls_general.is_empty_var(check_method) === 0) {
      //VERIFICA QUE EL METODO INGRESADO EXISTA
      cls_general.shot_toast_bs("M&eacute;todo erroneo.", {
        bg: "text-bg-warning",
      });
      return false;
    }
    ...
    if (total <= received) {
      ...
      if (received + amount > total) {
        if (check_method.tx_paymentmethod_change === 0) {
          cls_general.shot_toast_bs(
            "M&eacute;todo de pago, no admite cambio.",
            { bg: "text-bg-warning" },
          );
          return false;
        }
    ...
    var taked_id = [];
    var taked_amount = 0;
    cls_payment.payment.map((pay, index) => {
      if (pay.method_id === method) {
        taked_id.push(index);
        taked_amount = parseFloat(pay.amount);
      }
    });
    if (taked_id.length > 0) {
      cls_payment.payment.splice(taked_id, 1, {
        method_id: method,
        method_name: check_method.tx_paymentmethod_value,
        number: number,
        amount: parseFloat(amount) + taked_amount,
      });
    } else {
      cls_payment.payment.push({
        method_id: method,
        method_name: check_method.tx_paymentmethod_value,
        number: number,
        amount: amount,
  ...
  render() {
    let content_payment = cls_payment.generate_payment_list(
      cls_payment.payment,
      cls_payment.giftcard,
    );
  ...
  generate_payment_list(raw_payment, raw_giftcard) {
    ...
    raw_payment.map((payment, index) => {
      var number =
        cls_general.is_empty_var(payment.number) === 0
          ? ""
          : "(" + payment.number + ")";
      payment_list += `
    ...
    raw_giftcard.map((giftcard, index) => {
      var number =
        cls_general.is_empty_var(giftcard.giftcard_number) === 0
          ? ""
    ...
    if (parseFloat(cls_charge.charge_request.total) > received) {
      var diference =
        parseFloat(cls_charge.charge_request.total) - parseFloat(received);
      ...
      if (parseFloat(cls_charge.charge_request.total) === received) {
        ...
        var diference = 0;
        var change =
          parseFloat(received) - parseFloat(cls_charge.charge_request.total);
...
class class_creditnote {
  ...
  render_inspect(info, raw_data) {
    ...
    document.getElementById("inspectModal_footer").innerHTML = footer;
    document.getElementById("inspectModal_title").innerHTML =
      "Inspeccionar Nota de Cr&eacute;dito";
  }
  async filter(str, limit) {
    document.getElementById("container_creditnote").innerHTML =
      cls_creditnote.generate_list(await cls_creditnote.look_for(str, limit));
  }
  look_for(str, limit) {
    return new Promise((resolve) => {
      clearTimeout(this.timer);
          ...
          for (const a in needles) {
            if (
              haystack[i]["tx_client_name"]
                .toLowerCase()
                .indexOf(needles[a].toLowerCase()) > -1 ||
              haystack[i]["tx_client_cif"]
                .toLowerCase()
                .indexOf(needles[a].toLowerCase()) > -1 ||
              haystack[i]["tx_creditnote_number"]
                .toLowerCase()
                .indexOf(needles[a].toLowerCase()) > -1
            ) {
              ocurrencys++;
  ...
  render(
    ...
    var check_methoddebit = payment.find((pay) => {
      return pay.payment_ai_paymentmethod_id === 3;
    });
    var btn_nullify =
      raw_creditnote.length === 0 &&
      payment.length === 1 &&
      check_methoddebit === undefined
        ? `<button type="button" id="btn_creditnoteNullify" class="btn btn-danger btn-lg" onclick="cls_creditnote.nullify('${charge_slug}')">Anular</button>`
        : "";
    ...
    document
      .getElementById("btn_creditnoteProcess")
      .addEventListener("click", () => {
  ...
  async generate_articlecharge(raw_article) {
    ...
    raw_article.map((commanddata) => {
      if (commanddata.commanddata_status === 1) {
        ...
        var raw_price = [
          {
            price: commanddata.commanddata_price,
            discount: commanddata.commanddata_discountrate,
            tax: commanddata.commanddata_taxrate,
            quantity: 1,
          },
        ];
  ...
  add(
    ...
    presentation_value,
    price,
    commanddata_quantity,
    article_id,
    presentation_id,
  ) {
    var returned = cls_creditnote.selected.find((commanddata) => {
      return commanddata.commanddata_id === commanddata_id;
    });
    var available =
      returned != undefined
        ? commanddata_quantity - parseFloat(returned.quantity)
        : commanddata_quantity;
    swal({
      title: article_value,
      ...
      var i = "";
      cls_creditnote.selected.find((commanddata, index) => {
        if (commanddata.commanddata_id === commanddata_id) {
          i = index;
        }
      });
      if (cls_general.is_empty_var(i) === 1) {
        ...
        cls_creditnote.selected.push({
          commanddata_id: commanddata_id,
          article_id: article_id,
      ...
      var content = cls_creditnote.generate_articleselected(
  ...
  process(charge_slug) {
    cls_general.disable_submit(
      document.getElementById("btn_creditnoteProcess"),
    );
    if (cls_creditnote.selected.length < 1) {
      cls_general.shot_toast_bs("Debe incluir productos.", {
        bg: "text-bg-warning",
      });
      return false;
    }
  ...
  generate_creditnote(raw_creditnote) {
    ...
    raw_creditnote.map((creditnote) => {
      var nully =
        creditnote.tx_creditnote_nullification === 1
          ? "Anulaci&oacute;n"
...
class class_cashoutput {
  openmodal() {
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        document.getElementById("container_cashoutputlist").innerHTML =
          cls_cashoutput.generate_cashoutput(obj.data.cashoutputlist);
        var date = cls_general.getDate();
        document.getElementById("cashoutputDateshow").innerHTML =
          cls_general.date_converter("ymd", "dmy", date[0]);
        document.getElementById("cashoutputDatefilter").value =
          cls_general.date_converter("ymd", "dmy", date[0]);
  ...
  save() {
    ...
    if (
      cls_general.is_empty_var(reason) === 0 ||
      cls_general.is_empty_var(amount) === 0
    ) {
      cls_general.shot_toast_bs("Debe ingresar un monto y un motivo valido.", {
        bg: "text-bg-warning",
      });
      return false;
    }
    if (isNaN(amount)) {
      cls_general.shot_toast_bs("Debe introducir un numero.", {
        bg: "text-bg-warning",
      });
      return false;
    }
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        document.getElementById("container_cashoutputlist").innerHTML =
          cls_cashoutput.generate_cashoutput(obj.data.outputlist);
        cls_general.shot_toast_bs(obj.message, { bg: "text-bg-success" });
...
class class_cashregister {
  ...
  create() {
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        document.getElementById("cashregisterDatefilter").value =
          cls_general.date_converter("ymd", "dmy", today[0]);
        var cashregister_list = cls_cashregister.generate_list(
          obj.data.cashregister.cashregister_list,
        );
        document.getElementById("container_cashregisterFiltered").innerHTML =
          "<span>Listado de Cierres de Caja</span>" + cashregister_list;

        var incomeCashout =
          cls_general.is_empty_var(obj.data.cashregister.cashoutput.in) === 0
            ? 0
            : obj.data.cashregister.cashoutput.in;
        var outcomeCashout =
          cls_general.is_empty_var(obj.data.cashregister.cashoutput.out) === 0
            ? 0
            : obj.data.cashregister.cashoutput.out;
        var nullifiedCashout =
          cls_general.is_empty_var(
            obj.data.cashregister.cashoutput.nullified,
          ) === 0
            ? 0
            : obj.data.cashregister.cashoutput.nullified;
        document.getElementById("span_totalCashout").innerHTML =
          "B/ " +
          cls_general.val_price(incomeCashout - outcomeCashout, 2, 1, 1);
        document.getElementById("span_incomeCashout").innerHTML =
          "B/ " + cls_general.val_price(incomeCashout, 2, 1, 1);
        document.getElementById("span_outcomeCashout").innerHTML =
          "B/ " + cls_general.val_price(outcomeCashout, 2, 1, 1);
        document.getElementById("span_nullifiedCashout").innerHTML =
          "B/ " + cls_general.val_price(nullifiedCashout, 2, 1, 1);

        var incomeCash =
          cls_general.is_empty_var(obj.data.cashregister.payment[1]) === 0
            ? 0
            : obj.data.cashregister.payment[1];
        var returnCash =
          cls_general.is_empty_var(obj.data.cashregister.returnpayment[1]) === 0
            ? 0
            : obj.data.cashregister.returnpayment[1];
        document.getElementById("span_totalCash").innerHTML =
          "B/ " + cls_general.val_price(incomeCash - returnCash, 2, 1, 1);
        document.getElementById("span_incomeCash").innerHTML =
          "B/ " + cls_general.val_price(incomeCash, 2, 1, 1);
        document.getElementById("span_returnCash").innerHTML =
          "B/ " + cls_general.val_price(returnCash, 2, 1, 1);

        var incomeCheck =
          cls_general.is_empty_var(obj.data.cashregister.payment[2]) === 0
            ? 0
            : obj.data.cashregister.payment[2];
        var returnCheck =
          cls_general.is_empty_var(obj.data.cashregister.returnpayment[2]) === 0
            ? 0
            : obj.data.cashregister.returnpayment[2];
        document.getElementById("span_totalCheck").innerHTML =
          "B/ " + cls_general.val_price(incomeCheck - returnCheck, 2, 1, 1);
        document.getElementById("span_incomeCheck").innerHTML =
          "B/ " + cls_general.val_price(incomeCheck, 2, 1, 1);
        document.getElementById("span_returnCheck").innerHTML =
          "B/ " + cls_general.val_price(returnCheck, 2, 1, 1);

        document.getElementById("span_totalDebitcard").innerHTML =
          cls_general.is_empty_var(obj.data.cashregister.payment[3]) === 0
            ? "B/ " + cls_general.val_price(0, 2, 1, 1)
            : "B/ " +
              cls_general.val_price(obj.data.cashregister.payment[3], 2, 1, 1);
        document.getElementById("span_incomeDebitcard").innerHTML =
          cls_general.is_empty_var(obj.data.cashregister.payment[3]) === 0
            ? "B/ " + cls_general.val_price(0, 2, 1, 1)
            : "B/ " +
              cls_general.val_price(obj.data.cashregister.payment[3], 2, 1, 1);
        document.getElementById("span_returnDebitcard").innerHTML =
          "B/ " + cls_general.val_price(0, 2, 1, 1);

        var incomeCreditcard =
          cls_general.is_empty_var(obj.data.cashregister.payment[4]) === 0
            ? 0
            : obj.data.cashregister.payment[4];
        var returnCreditcard =
          cls_general.is_empty_var(obj.data.cashregister.returnpayment[4]) === 0
            ? 0
            : obj.data.cashregister.returnpayment[4];
        document.getElementById("span_totalCreditcard").innerHTML =
          "B/ " +
          cls_general.val_price(incomeCreditcard - returnCreditcard, 2, 1, 1);
        document.getElementById("span_incomeCreditcard").innerHTML =
          "B/ " + cls_general.val_price(incomeCreditcard, 2, 1, 1);
        document.getElementById("span_returnCreditcard").innerHTML =
          "B/ " + cls_general.val_price(returnCreditcard, 2, 1, 1);

        var incomeYappi =
          cls_general.is_empty_var(obj.data.cashregister.payment[5]) === 0
            ? 0
            : obj.data.cashregister.payment[5];
        var returnYappi =
          cls_general.is_empty_var(obj.data.cashregister.returnpayment[5]) === 0
            ? 0
            : obj.data.cashregister.returnpayment[5];
        document.getElementById("span_totalYappi").innerHTML =
          "B/ " + cls_general.val_price(incomeYappi - returnYappi, 2, 1, 1);
        document.getElementById("span_incomeYappi").innerHTML =
          "B/ " + cls_general.val_price(incomeYappi, 2, 1, 1);
        document.getElementById("span_returnYappi").innerHTML =
          "B/ " + cls_general.val_price(returnYappi, 2, 1, 1);

        var incomeNequi =
          cls_general.is_empty_var(obj.data.cashregister.payment[6]) === 0
            ? 0
            : obj.data.cashregister.payment[6];
        var returnNequi =
          cls_general.is_empty_var(obj.data.cashregister.returnpayment[6]) === 0
            ? 0
            : obj.data.cashregister.returnpayment[6];
        document.getElementById("span_totalNequi").innerHTML =
          "B/ " + cls_general.val_price(incomeNequi - returnNequi, 2, 1, 1);
        document.getElementById("span_incomeNequi").innerHTML =
          "B/ " + cls_general.val_price(incomeNequi, 2, 1, 1);
        document.getElementById("span_returnNequi").innerHTML =
          "B/ " + cls_general.val_price(returnNequi, 2, 1, 1);

        var incomeAnother =
          cls_general.is_empty_var(obj.data.cashregister.payment[7]) === 0
            ? 0
            : obj.data.cashregister.payment[7];
        var returnAnother =
          cls_general.is_empty_var(obj.data.cashregister.returnpayment[7]) === 0
            ? 0
            : obj.data.cashregister.returnpayment[7];
        document.getElementById("span_totalAnother").innerHTML =
          "B/ " + cls_general.val_price(incomeAnother - returnAnother, 2, 1, 1);
        document.getElementById("span_incomeAnother").innerHTML =
          "B/ " + cls_general.val_price(incomeAnother, 2, 1, 1);
        document.getElementById("span_returnAnother").innerHTML =
          "B/ " + cls_general.val_price(returnAnother, 2, 1, 1);

        var incomeGiftcard =
          cls_general.is_empty_var(obj.data.cashregister.payment[8]) === 0
            ? 0
            : obj.data.cashregister.payment[8];
        var returnGiftcard =
          cls_general.is_empty_var(obj.data.cashregister.returnpayment[8]) === 0
            ? 0
            : obj.data.cashregister.returnpayment[8];
        document.getElementById("span_totalGiftcard").innerHTML =
          "B/ " +
          cls_general.val_price(incomeGiftcard - returnGiftcard, 2, 1, 1);
        document.getElementById("span_incomeGiftcard").innerHTML =
          "B/ " + cls_general.val_price(incomeGiftcard, 2, 1, 1);
        document.getElementById("span_returnGiftcard").innerHTML =
          "B/ " + cls_general.val_price(returnGiftcard, 2, 1, 1);
        ...
        obj.data.cashregister.giftcard.map((giftcard) => {
          ...
          var ttl_giftcard = 0;
          giftcard_payment.map((gp) => {
            ttl_giftcard += gp.amount;
          });
        ...
        var activeGiftcard =
          cls_general.is_empty_var(total_giftcardactive) === 0
            ? 0
            : total_giftcardactive;
        var inactiveGiftcard =
          cls_general.is_empty_var(total_giftcardinactive) === 0
            ? 0
            : total_giftcardinactive;
        document.getElementById("span_giftcardactive").innerHTML =
          "<p>Cupones Activos</p>B/ " +
          cls_general.val_price(activeGiftcard, 2, 1, 1);
        document.getElementById("span_giftcardinactive").innerHTML =
          "<p>Cupones inactivos</p>B/ " +
          cls_general.val_price(inactiveGiftcard, 2, 1, 1);

        document.getElementById("span_totaltip").innerHTML =
          "B/ " + cls_general.val_price(obj.data.cashregister.tip, 2, 1, 1);
        document.getElementById("span_totaldiscount").innerHTML =
          "B/ " +
          cls_general.val_price(obj.data.cashregister.discount, 2, 1, 1);
        document.getElementById("span_totalnull").innerHTML =
          "B/ " +
          cls_general.val_price(obj.data.cashregister.canceled, 2, 1, 1);
        document.getElementById("span_totalcashback").innerHTML =
          "B/ " +
          cls_general.val_price(obj.data.cashregister.cashback, 2, 1, 1);
        document.getElementById("span_grosssale").innerHTML =
          "B/ " +
          cls_general.val_price(obj.data.cashregister.grosssale, 2, 1, 1);
        document.getElementById("span_netsale").innerHTML =
          "B/ " + cls_general.val_price(obj.data.cashregister.netsale, 2, 1, 1);
        document.getElementById("span_realsale").innerHTML =
          "B/ " +
          cls_general.val_price(obj.data.cashregister.realsale, 2, 1, 1);
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        document.getElementById("container_cashregisterFiltered").innerHTML =
          ...
          cls_cashregister.generate_list(obj.data.all);
      } else {
        cls_general.shot_toast_bs(obj.message, { bg: "text-bg-warning" });
      }
    };
  ...
  generate_inspectedCR(cashregister, payment, canceled, giftcard, charge_list) {
    var title =
      "Arqueo del " +
      cls_general.datetime_converter(cashregister.created_at) +
      " a las " +
      cls_general.time_converter(cashregister.created_at);
    var income_cash =
      cls_general.is_empty_var(payment[1]) === 1 ? payment[1] : 0;
    var outcome_cash =
      cls_general.is_empty_var(canceled[1]) === 1 ? payment[1] : 0;

    var income_check =
      cls_general.is_empty_var(payment[2]) === 1 ? payment[2] : 0;
    var outcome_check =
      cls_general.is_empty_var(canceled[2]) === 1 ? payment[2] : 0;

    var income_debitcard =
      cls_general.is_empty_var(payment[3]) === 1 ? payment[3] : 0;
    var outcome_debitcard =
      cls_general.is_empty_var(canceled[3]) === 1 ? payment[3] : 0;

    var income_creditcard =
      cls_general.is_empty_var(payment[4]) === 1 ? payment[4] : 0;
    var outcome_creditcard =
      cls_general.is_empty_var(canceled[4]) === 1 ? payment[4] : 0;

    var income_yappi =
      cls_general.is_empty_var(payment[5]) === 1 ? payment[5] : 0;
    var outcome_yappi =
      cls_general.is_empty_var(canceled[5]) === 1 ? payment[5] : 0;

    var income_nequi =
      cls_general.is_empty_var(payment[6]) === 1 ? payment[6] : 0;
    var outcome_nequi =
      cls_general.is_empty_var(canceled[6]) === 1 ? payment[6] : 0;

    var income_another =
      cls_general.is_empty_var(payment[7]) === 1 ? payment[7] : 0;
    var outcome_another =
      cls_general.is_empty_var(canceled[7]) === 1 ? payment[7] : 0;

    var income_giftcard =
      cls_general.is_empty_var(payment[8]) === 1 ? payment[8] : 0;
    var outcome_giftcard =
      cls_general.is_empty_var(canceled[8]) === 1 ? payment[8] : 0;
  ...
  show(cashregister_id) {
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        ...
        var raw_inspected = cls_cashregister.generate_inspectedCR(
          obj.data.cashregister,
          obj.data.payment,
          obj.data.canceled,
          total_giftcard,
          obj.data.charge,
        );
        document.getElementById("inspectCR_title").innerHTML =
          raw_inspected.title;
        document.getElementById("inspectCR_container").innerHTML =
          raw_inspected.content + raw_inspected.detail;
...
class class_client {
  ...
  generate_list(client_list, limit) {
    ...
    for (const a in client_list) {
      ...
      var bg =
        client_list[a]["tx_client_status"] == "0" ? "text-bg-secondary" : "";
      var inactive =
        client_list[a]["tx_client_status"] == "0" ? " (INACTIVO)" : "";
  ...
  async filter(str, limit) {
    document.getElementById("container_clientList").innerHTML =
      cls_client.generate_list(await cls_client.look_for(str, limit));
  }
  look_for(str, limit) {
    return new Promise((resolve) => {
      clearTimeout(this.timer);
  ...
  show(client_slug) {
    ...
    var funcion = function (obj) {
      ...
      var dv =
        cls_general.is_empty_var(client.tx_client_dv) === 0
          ? ""
          : client.tx_client_dv;
      var telephone =
        cls_general.is_empty_var(client.tx_client_telephone) === 0
          ? ""
          : client.tx_client_telephone;
      var email =
        cls_general.is_empty_var(client.tx_client_email) === 0
          ? ""
          : client.tx_client_email;
      var direction =
        cls_general.is_empty_var(client.tx_client_direction) === 0
          ? ""
          : client.tx_client_direction;
      var content = `
        <div class="row">
          <div class="col">
            <div class="row">
      ...
      `;
      document.getElementById("container_request").innerHTML =
        content + content_bottom;
      document.getElementById("clientTaxpayer").value =
        client.tx_client_taxpayer + "" + client.tx_client_type;
    };
    cls_general.async_laravel_request(url, method, funcion, body);
  }
  create() {
  ...
  async filter_modal(str) {
    document.getElementById("container_clientfiltered").innerHTML =
      cls_client.generate_modal_clientlist(await cls_client.look_for(str, 200));
  }
  render_modal() {
    var btn_update_client =
      cls_general.is_empty_var(
        document.getElementById("requestClient").name,
      ) === 0 || document.getElementById("requestClient").name === "1"
        ? ""
        : '<button type="button" class="btn btn-warning" onclick="cls_client.render_modal_edit()">Modificar Cliente</button>';
  ...
  render_modal_edit() {
    ...
    var funcion = function (obj) {
      var client = obj.data.client;
      if (obj.status === "success") {
        cls_client.render_modal_create(
          client.tx_client_slug,
          client.tx_client_name,
          client.tx_client_cif,
          client.tx_client_dv,
          client.tx_client_telephone,
          client.tx_client_email,
          client.tx_client_direction,
          client.tx_client_exempt,
          client.tx_client_taxpayer + client.tx_client_type,
          client.tx_client_status,
        );
      } else {
        cls_general.shot_toast_bs(obj.message, { bg: "text-bg-warning" });
      }
...
class class_giftcard {
  ...
  index(client_slug) {
    ...
    var funcion = function (obj) {
      if (obj.status === "success") {
        ...
        document.getElementById("new_giftcard").name =
          obj.data.client.ai_client_id;
        document.getElementById("container_giftcard_active").innerHTML =
          cls_giftcard.generate_active(cls_giftcard.active);
        document.getElementById("container_giftcard_inactive").innerHTML =
          cls_giftcard.generate_inactive(cls_giftcard.inactive);
      } else {
        cls_general.shot_toast_bs(obj.message, { bg: "text-bg-warning" });
      }
    };
  ...
  save(client_slug) {
    ...
    if (received < 0.25) {
      cls_general.shot_toast_bs("Debe ingresar un monto superior a 0.25", {
        bg: "text-bg-warning",
      });
      return false;
    }
  ...
  select_giftcard() {
    ...
    swal({
      ...
      var check_duplicity = cls_payment.giftcard.find((payment) => {
        return payment.giftcard_number === number;
      });
      ...
      var funcion = function (obj) {
        if (obj.status === "success") {
          if (amount > obj.data.info.tx_giftcard_amount) {
            cls_general.shot_toast_bs(
              `No alcanza el saldo disponible. (Disponible: ${cls_general.val_dec(obj.data.info.tx_giftcard_amount, 2, 1, 1)})`,
              { bg: "text-bg-warning" },
            );
            return false;
          }
